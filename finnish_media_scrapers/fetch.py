...
but HS/AL/SK needs a user to be logged in, as well as renders their articles using dynamic javascript,
...
async def fetch_article_al(
        session: Page,
        url: str, max_web_driver_wait: int = 30) -> str:
    """Fetch the HTML of a single article using a pyppeteer session where
    prepare_session_al has been called before.

    Args:
        session (Page): the pyppeteer session to use
        url (str): the AL article URL to fetch article content from
        max_web_driver_wait (int): the maximum number of seconds to wait for the webdriver to
                                   render a page before failing (default: 30)
    ...
    try:
        ...
        raise ValueError(
            f"Couldn't find the dynamic content I was looking for in {url}. There may be a class of AL articles we're not yet handling."
        ) from timeout_exception
    article = await content.content()
    return article


async def prepare_session_al(
        session: Page,
        username: str, password: str, max_web_driver_wait: int = 30):
    """Prepare a pyppeteer session for scraping articles from Aamulehti
    by logging in using the provided user id and password.
    ...
    """
    max_web_driver_wait = 1000 * max_web_driver_wait
    await session.goto("https://www.aamulehti.fi", timeout=max_web_driver_wait)
    cookies_frame = await session.waitForXPath("//iframe[@title='SP Consent Message']", timeout=max_web_driver_wait)
    frame = await cookies_frame.contentFrame()
    ok_button = await frame.waitForXPath("//button[@title='OK']", timeout=max_web_driver_wait)
    await ok_button.click()
...
async def fetch_article_sk(
        session: Page,
        url: str, max_web_driver_wait: int = 30) -> str:
    """Fetch the HTML of a single article using a pyppeteer session where
    prepare_session_sk has been called before.

    Args:
        session (Page): the pyppeteer session to use
        url (str): the SK article URL to fetch article content from
        max_web_driver_wait (int): the maximum number of seconds to wait for the webdriver to
                                   render a page before failing (default: 30)
...
async def prepare_session_sk(
        session: Page,
        username: str, password: str, max_web_driver_wait: int = 30):
    """Prepare a pyppeteer session for scraping articles from Satakunnan kansa
    by logging in using the provided user id and password.
    ...
    """
    max_web_driver_wait = 1000 * max_web_driver_wait
    await session.goto("https://www.satakunnankansa.fi", timeout=max_web_driver_wait)
    cookies_frame = await session.waitForXPath("//iframe[@title='SP Consent Message']", timeout=max_web_driver_wait)
    frame = await cookies_frame.contentFrame()
    ok_button = await frame.waitForXPath("//button[@title='OK']", timeout=max_web_driver_wait)
    await ok_button.click()

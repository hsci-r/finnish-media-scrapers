...
def parse_arguments():
    ...
    parser.add_argument(
        '-w', '--web-driver', help="the Selenium web driver to use (out of remote [default],chrome,firefox)", default="remote")
    parser.add_argument(
    ...
    try:
        if args.web_driver == 'remote':
            try:
                ...
                logging.error(
                    "Cannot download articles if Selenium is not running on localhost. (e.g. docker run -p 4444:4444 -v /dev/shm:/dev/shm selenium/standalone-chrome:4.0.0-beta-1-20210215)")
                return
            ...
            logging.error(
                "Unknown web driver specified. Must be one of remote/chrome/firefox.")
            return
    except WebDriverException:
        ...
        with open(args.input) as input_file:
            ...
            for article in csv_input:
                ...
                if not os.path.exists(file):
                    article = fetch_article_hs(
                        driver, url, args.max_web_driver_wait)

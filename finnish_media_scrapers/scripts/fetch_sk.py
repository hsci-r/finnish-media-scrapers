#!/usr/bin/env python3
"""Command-line script for fetching article HTML from Satakunnan kansa
"""
# %%
...
from ..fetch import fetch_article_sk, prepare_session_sk
...
def _parse_arguments():
    ...
    parser.add_argument(
        '-i', '--input', help="input CSV file containing articles to fetch (from query-sk.py)", required=True)
    parser.add_argument(
...
async def _amain():
    ...
    try:
        ...
        try:
            logging.info("Logging into SK.")
            await prepare_session_sk(session, args.username,
            ...
            with open(args.input, encoding="utf-8") as input_file:
                ...
                for article in csv_input:
                    ...
                    if not os.path.exists(file):
                        try:
                            article = await fetch_article_sk(

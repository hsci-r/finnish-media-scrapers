#!/usr/bin/env python3
"""Command-line script for fetching article HTML from Aamulehti
"""
# %%
...
from ..fetch import fetch_article_al, prepare_session_al
...
def _parse_arguments():
    ...
    parser.add_argument(
        '-i', '--input', help="input CSV file containing articles to fetch (from query-al.py)", required=True)
    parser.add_argument(
...
async def _amain():
    ...
    try:
        ...
        try:
            logging.info("Logging into AL.")
            await prepare_session_al(session, args.username,
            ...
            with open(args.input, encoding="utf-8") as input_file:
                ...
                for article in csv_input:
                    ...
                    if not os.path.exists(file):
                        try:
                            article = await fetch_article_al(

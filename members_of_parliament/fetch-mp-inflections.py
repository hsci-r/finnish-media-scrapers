...
parser.add_argument('-u', '--username',
                    help="email to use for article fetching")
parser.add_argument('-p', '--password',
                    help="password to use for article fetching")
...
for i in range(0, len(names)):
    ...
    os.system('fms-fetch-open -i ' + filename +
              ' -o ../articles/il-' + original_name_without_spaces)

    # IS
    filename = '../queries/is-inflections-' + name_without_spaces + ".csv"
    os.system('fms-fetch-open -i ' + filename +
              ' -o ../articles/is-' + original_name_without_spaces)

    # YLE
    filename = '../queries/yle-inflections-' + name_without_spaces + ".csv"
    os.system('fms-fetch-open -i ' + filename +
              ' -o ../articles/yle-' + original_name_without_spaces)

    # HS
    filename = '../queries/hs-inflections-' + name_without_spaces + ".csv"
    os.system('fms-fetch-hs -i ' + filename + ' -o ../articles/hs-' +
              original_name_without_spaces + " -u " + args.username + " -p " + args.password)
